...
public class WOD5ExtTplObj {
	...
	public WOD5ExtTplObj(/*IGenomes*/ MpiWsService service, Request req, Response resp,
			IContentNode node) {
	...
	public String render() {
		...
		try {
			float  lon = 0; //req.get("lon");
			float  lat = 0; //req.get("lat");
			float depth = 0; //req.get("depth");
			String param = ""; //req.get("param");
			float buffer = 0; //req.get("season");
			List<WOD5> w = service.wod05_idw_ip(lon, lat, depth, buffer, param);
		...
		//return resp.formatTemplate("megx.net/ext/table/wod5.html", params);
