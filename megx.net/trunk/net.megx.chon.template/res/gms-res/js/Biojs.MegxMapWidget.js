 ...
 * 
 * @option {string} [gmsBaseURL="http://mb3is.megx.net/wms"] Base URL.
 * 
...
Biojs.MegxMapWidget = Biojs
        .extend({
            ...
            _init : function(options) {
                ...
                if (layerset) {
                    for ( var i = 0; i < layerset.length; i++) {
                        var addMap = this.map.addLayer(this.layers
                                .get(layerset[i]));
                        if (addMap) {
                            ...
                            this._log
                                    .error('An error occured while adding layer '
                                            + layerset[i] + ' to map!');
                        }
                    }
                }
                this._createWMSFeatureInfo(this.layers.get('ena_samples'),
                        this.map);
             ...
             * @param {object}
             *            map Description.
             * @example _createWMSFeatureInfo(this.layers.get('ena_samples'),
             *          this.map);
             * 
             */
            _createWMSFeatureInfo : function(layer, map) {
                ...
                var featureInfo = new OpenLayers.Control.WMSGetFeatureInfo({
                    /*
                     * vendorParams : { RRNAS : sam_rrna, GENOMES : sam_genomes,
                     * PHAGES : sam_phages, METAGENOMES : sam_metagenomes },
                     */
                    url : this.gmsBaseURL,
                    queryVisible : true,
                    ...
                    eventListeners : {
                        getfeatureinfo : function(event) {
                            ...
                            map.addPopup(new OpenLayers.Popup.FramedCloud(
                                    "chicken", lonlat, null,
                                    event.text ? event.text : 'No data at '
                                            + event.text + ': ' + lonlat
                                            + event.features + event.xy, null,
                                    true));
                        }
                    }
                });
            ...
            _renderWidget : function(layoutParent) {
                ...
                var layoutHtml = [
                        ...
                        '<td colspan="2" style="padding-left: 16%;">',
                        '<legend>', '<h6>', 'Legend', '</h6>',
                        '<div id="legendDataPlaceholder">', '</div>',
                        '</legend>', '</td>', '</tr>', '</table>',
                        '<div id="layerDialog">', '</div>' ].join('');
            ...
            _log : {
                message : function(msg) {
                    ...
                    var msgToDisplay = [
                            '<p style="color: green; padding-left: 10px;">',
                            timeStamp.toLocaleString(), ' ', msg, '</p>' ]
                            .join('');
                    var nbDisplayedMsgs = $('#messagesPlaceholder').children().length;
                    var childrenHeight = $('#messagesPlaceholder').children()
                            .height();
                        ...
                        .dialog(
                                {
                                    ...
                                    buttons : {
                                        Ok : function() {
                                            self._addLayers();
                                            self._log
                                                    .message('Closing dialog window. New layer is added on map');
                                            $(this).dialog("close");
                                        },
                                        Cancel : function() {
                                            self._log
                                                    .message('Closing dialog window. No layer is added on map');
                                            $(this).dialog("close");
                                        }
                                    }
                                });
            ...
            _initializeAddRemoveLink : function() {
                ...
                $(document).on(
                        ...
                        'button.removeLayer',
                        function() {
                            var layerToRemove = $(this).closest('div.mx-layer')
                                    .attr('id');
                            self._removeLayer(layerToRemove);
                            self._removeLayerPanel(layerToRemove);
                            self._log.message('Layer ' + layerToRemove
                                    + ' removed from map');
                        });
            },
            ...
            _populateLayersDialog : function() {
                ...
                for ( var layer in allLayers) {
                    ...
                    if (!this.displayedLayers[currentLayer.genericName]) {
                        htmlToRender
                                .push('<input class="layer" type="radio" value="'
                                        + currentLayer.genericName
                                        + '" name="layerChoice" />'
            ...
            _addLayerPanel : function(layer, success) {
                ...
                var layerPanelHtml = [
                        ...
                        '<p class="mx-layer-description">'
                                + layerInfo.description + '</p>',
                        '<div class="mx-layer-control"></div>', '</div>',
                        '</div>' ].join('');
            ...
            _initMap : function(map) {
                ...
                var permalinkContr = new OpenLayers.Control.Permalink(
                        'permalink');
            ...
            _reorder : function(arr) {
                ...
                this._log
                        .message('Reordering layers as per new layer order...');
