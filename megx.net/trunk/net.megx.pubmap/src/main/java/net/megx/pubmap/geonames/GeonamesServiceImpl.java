  ...
  public Place getPlaceName(String lat, String lon) {
    ...
    try {
      uri = new URIBuilder().setScheme("http").setHost("api.geonames.org")
          .setPath("/extendedFindNearby").setParameter("lat", lat)
          .setParameter("lng", lon).setParameter("username", "megx").build();
        ...
        if (entity != null) {
          ...
          if (geonames.getGeonamesLst() != null) {
            ...
            for (Geoname geoname : geonamesList) {
              ...
              distance = Math
                  .sqrt(Math.pow(
                      (requestLon - Double.valueOf(geoname.getLng())), 2)
                      + Math.pow(
                          (requestLat - Double.valueOf(geoname.getLat())), 2));
