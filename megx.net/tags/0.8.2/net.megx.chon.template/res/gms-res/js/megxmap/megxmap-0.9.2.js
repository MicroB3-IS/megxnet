...
Biojs.MegxMapWidget = Biojs
  .extend({
    ...
    _init : function( options ) {
      ...
      // options
      Biojs.MegxMapWidget.prototype._log.enabled = this.opt.log ? this.opt.log : false;

      this._renderWidget(this.opt.target);

      this._log.error('MegxMapWidget 0.9.2');
      ...
      if ( layerset ) {
        ...
        this._log.error('Requested set of layers does not exist: '
    ...
    _createWMSFeatureInfo : function( layer, map ) {
      ...
      self.map.addControl(featureInfo);
    ...
    _renderWidget : function( layoutParent ) {
      ...
      var legendHTML = [ '<legend>', '<h6>', 'Legend', '</h6>',
        '<div id="legendDataPlaceholder">', '</div>', '</legend>' ].join('');
