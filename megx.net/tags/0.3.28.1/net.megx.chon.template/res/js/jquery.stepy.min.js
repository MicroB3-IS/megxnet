...
(function(b) {
	var a = {
		init : function(c) {
			return this
					.each(function() {
						var g = b.extend({}, b.fn.stepy.defaults, c), m = b(
								this).data("options", g), f = m.attr("id");
						if (f === undefined || f == "") {
						...
						if (g.titleClick) {
							d
									.click(function() {
										var s = d.filter(".current-step").attr(
												"id").split("-"), r = parseInt(
												s[s.length - 1], 10), q = b(
												this).index();
										if (q > r) {
						...
						l.first().find(":input:visible:enabled").first()
								.select().focus();
					});
		},
		...
		validate : function(d) {
			...
			var c = this.children("fieldset").eq(d), h = true, f = b(
					"#" + this.attr("id") + "-titles").children().eq(d), e = this
					.data("options"), g = this.validate();
