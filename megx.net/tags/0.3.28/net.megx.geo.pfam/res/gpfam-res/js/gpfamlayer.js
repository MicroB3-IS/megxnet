...
// var pfam_selection = '<select id="depth" name="depth"
// onchange="setPFAM(this.options[this.selectedIndex].value)">'
...
function cMap() {
    ...
    samplingsitesLayer = new OpenLayers.Layer.WMS(
            "samplingsites",
            gms_wms_url,
            {
        ...
        eventListeners : {
            getfeatureinfo : function(event) {
                ...
                map.addPopup(new OpenLayers.Popup.FramedCloud("chicken",
                        lonlat, null, event.text ? event.text : 'No result at '
                                + lonlat, null, true));
            }
        }
    });
    ...
    OpenLayers.Console.log("geopfam index=" + map.getLayerIndex(pfamLayer));
...
function refreshPFAMLayer() {
    OpenLayers.Console.log('refreshing Genes Mapserver;pfam_accession='
            + cur_season);
...
function resetMap() {
    var h = document.getElementById('permalink');
    if (h != null) {
        // leaving out the hash part in order to really reload the original
        // status
...
function getPFAM() {
    ...
    if (e != '') {
        ...
        $.ajax({
            ...
            success : function(i) {
                var n = $(i), o = n.find("description").text(), r = n.find(
                        "comment").text(), s = $("<a></a>").attr("href",
                        pfamBase + e).attr("target", "_blank").addClass(
                        "titleLink").text(e);

                o.length > 0 || r.length > 0 ? ($("#pfam-description").html(
                        $("<p></p>").html(s).append(o)).append($("<p></p>")
                        .html(r))) : ($("#pfam-description"))
            },
            error : function() {
                $("#pfam-description").text(
                        "Error querying web service for XML.")
            }
        })
