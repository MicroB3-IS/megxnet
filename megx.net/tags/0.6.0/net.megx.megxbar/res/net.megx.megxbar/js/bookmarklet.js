(function Megxbookmarklet() {
	if (!window.megxbookmarklet)
		window.megxbookmarklet = {};
	var body, ifrm, closeButton, closeButtonDiv;
	var iframeHolder = (function(){
		var ifr = null;
		return {
			setIframe: function(i){ifr = i},
	...
	function createIframe() {
		...
		ifrm.src = "http://iwgate.poweredbyclear.com:9080/megx.net/bookmark";
		ifrm.allowTransparency = true;
		ifrm.frameBorder = 0;
		useCss(ifrm, iframeCss);
		closeButtonDiv = createCloseButton();
		var bookmarkletDiv = document.createElement("DIV");
	        ...
	        if (msg.type && msg.type == "getarticle") {
	            ...
	            jQuery.get(document.URL + "?report=xml&format=text", function(data) {
				    ...
				    source.postMessage(JSON.stringify({
		                type: "setarticle",
		                article: article
		            }), "http://iwgate.poweredbyclear.com:9080");
	...
	var listener = function(event) {
	    ...
	    } else if (event.origin == "http://iwgate.poweredbyclear.com:9080") {
