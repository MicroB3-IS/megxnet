...
public class MGTraitsAPI extends BaseRestService {
    ...
    @Produces("text/csv")
    @CSVDocument(preserveHeaderColumns = true, columnNameFormat = ColumnNameFormat.FROM_CAMEL_CASE)
    public List<MGTraitsCodon> getCodonUsage(@PathParam("id") int id,
            @PathParam("sample_name") String sampleName,
            @Context HttpServletRequest request) {
        try {
    ...
    public String getCodonUsageAsJSON(@PathParam("id") int id,
            ...
            @Context HttpServletRequest request) {
        try {
            return toJSON(new Result<List<MGTraitsCodon>>(
                    service.getCodonUsage(id)));
        } catch (DBGeneralFailureException e) {
            throw new WebApplicationException(e,
                    Response.Status.INTERNAL_SERVER_ERROR);
        } catch (DBNoRecordsException e) {
    ...
    @Produces("text/csv")
    @CSVDocument(preserveHeaderColumns = true, columnNameFormat = ColumnNameFormat.FROM_CAMEL_CASE)
    public TaxonomicContentToClient getTaxonomicContent(
            @PathParam("id") int id,
            @PathParam("sample_name") String sampleName,
            @Context HttpServletRequest request) {
        try {
            return new TaxonomicContentToClient(service.getTaxonomyContent(id));
        } catch (DBGeneralFailureException e) {
            throw new WebApplicationException(e,
                    Response.Status.INTERNAL_SERVER_ERROR);
        } catch (DBNoRecordsException e) {
