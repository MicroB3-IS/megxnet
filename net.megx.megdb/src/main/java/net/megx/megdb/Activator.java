...
public class Activator extends JCRAppConfgEnabledActivator {
  ...
  protected BaseMegdbService registerBaseDBService(
    ...
    try {
      ...
      log.fatal("Could not instantiate service: " + registerAs.getName(), e);
  ...
  protected void setupPasswordHash(JSONObject config) {
    ...
    if (passwordHashing != null) {
      ...
      int iterations = config.optInt("PBKDF2iterations",
      ...
      if (log.isDebugEnabled())
        log.debug(String
            .format(
                "Password Hashing Options: saltBytes=%d, hashBytes=%d, iterations=%d",
                saltBytes, hashBytes, iterations));
    } else {
