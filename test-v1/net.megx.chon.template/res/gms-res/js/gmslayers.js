...
function cMap() {
	...
	samplingsitesLayer.setVisibility(true);
	...
	setVisible(satelliteLayerMod.name, true);
	...
	// create the legend
	cLegend('samplingsites');

	var url = samplingsitesLayer.getFullRequestString();
	...
	info = new OpenLayers.Control.WMSGetFeatureInfo({
				...
				layers : map.getLayersByName('samplingsites'),
	...
	OpenLayers.Console.log("Sampling Sites request="
			+ samplingsitesLayer.getFullRequestString());
	OpenLayers.Console.log("bathyindex=" + map.getLayerIndex(bathymetryLayer));
	OpenLayers.Console.log("samsite index="
			+ map.getLayerIndex(samplingsitesLayer));

	setSamplingSiteFilter('genome',
			document.getElementById('genomes_cb').checked);
	setSamplingSiteFilter('metagenome', document
					.getElementById('metagenomes_cb').checked);
	setSamplingSiteFilter('phage', document.getElementById('phages_cb').checked);
	setSamplingSiteFilter('rRNA', document.getElementById('rrnas_cb').checked);
...
/**
 * Additional layer and interactivity Geographic-BLAST
 *
 * @param jobsid
 *            The session id of the job
 * @param jobid
 *            Random job id assigned at the time of submission
 * @return
 */
function cBlastMap(jobsid, jobid) {
	sid = jobsid;
	jid = jobid;
	if (sid == null || jid == null) {
		alert("can not create Geographic-BLAST map, because session paramters are missing!");
		return false;
	}
	// create Genes Mapserver
	if (!cMap()) {
		return false;
	}
	samplingsitesLayer.setVisibility(false);

	setSamplingSiteFilter('genome', false);
	setSamplingSiteFilter('metagenome', false);
	setSamplingSiteFilter('phage', false);

	var lname = 'blast_hit_counts';

	var blastHitCountLayer = new OpenLayers.Layer.WMS("blast_hit_counts",
			gms_wms_url, {
				layers : lname,
				format : 'image/png',
				transparent : "true",
	...
	var blastHitLayer = new OpenLayers.Layer.WMS("blast_hits", gms_wms_url, {
				layers : 'blast_hits',
				format : 'image/png',
				transparent : "true",
				isBaseLayer : false,
	...
	samplingsitesLayer.setVisibility(false);
	blastHitCountLayer.setVisibility(true);
	blastHitLayer.setVisibility(false);
	map.addLayer(blastHitCountLayer);
	map.addLayer(blastHitLayer);

	cLegend(lname);
	info.deactivate();
	var blastInfo = new OpenLayers.Control.WMSGetFeatureInfo({
				// url : blastHitLayer.getFullRequestString(),
				// hover: true,
				layers : [blastHitLayer],
				title : 'Identify features by clicking',
				vendorParams : {
					SID : jobsid,
					JID : jobid
				...
				eventListeners : {
					getfeatureinfo : function(event) {
						...
						map.addPopup(new OpenLayers.Popup.FramedCloud(
								"chicken", lonlat, null, event.text
										? event.text
										: 'No hits at ' + lonlat, null, true));
					}
				}
			});
	map.addControl(blastInfo);
	blastInfo.activate();
	setLocationbarHash();
	c_mode = 'blast';
	// OpenLayers.Console.log("blast for:" + jobsid + "," + jobid + "="
	// + blastHitLayer.getFullRequestString());
	// OpenLayers.Console.log("control:" + OpenLayers.Console.dir(blastInfo));
}

function cMetabarMap() {
	...
	var mStart = getURLParam('start');
	var mStop = getURLParam('stop');
	var mUser = getURLParam('user');
	// do validation of params

	metabarLayer = new OpenLayers.Layer.WMS("metabar", gms_wms_url, {
				layers : 'metabar',
				format : 'image/png',
				transparent : "true",
				username : mUser,
	...
	map.addLayer(metabarLayer);
	metabarLayer.setVisibility(true);

	OpenLayers.Console.log("metabar added="
			+ metabarLayer.getFullRequestString());
}
...
function reloadSamplingSitesLayer() {
	var l = map.getLayersByName('samplingsites');
	// only get the first hit
	if (l != null) {
		var layer = l[0];
		// layer.redraw(true);
		...
		// now also have to reset the GetFeatureinfo params:
		info.vendorParams = {
			RRNAS : sam_rrna,
			GENOMES : sam_genomes,
			PHAGES : sam_phages,
...
function cLegend(layerName) {
	...
	if (layerName == null || layerName == '') {
		document.getElementById(legendId).innerHTML = 'uyuyuyu';
	} else {
